...
void motionplanning::rmlPos(Eigen::VectorXd cp,Eigen::VectorXd tp,float maxVel, float maxAccel)
{
    ...
    for (int i = 0; i < m_Dofs; i++)
    {
        ...
        m_IP->MaxVelocityVector->VecData[i] = (maxVel == -1)?m_JointMaxVel:maxVel;
        m_IP->MaxAccelerationVector->VecData[i] = (maxAccel == -1)?m_JointMaxAccel:maxAccel;
        m_IP->MaxJerkVector->VecData[i] = 1;
